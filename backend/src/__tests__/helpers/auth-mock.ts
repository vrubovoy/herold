...
// service whose audience/scope they name - see Schrank's own identical
// helpers/auth-mock.ts for the sibling precedent this was ported from.
...
export const requireExportAuth = createMiddleware<MockExportAuthEnv>(async (c, next) => {
  ...
  const token = auth.slice(7)
  const user = token === 'herold-export-delegation-token'
    ? TOKENS['test-token']
    : token === 'herold-export-user2-delegation-token'
      ? TOKENS['user2-token']
      : TOKENS[token]
  if (!user) {
    return c.json({ error: 'Unauthorized' }, 401)
